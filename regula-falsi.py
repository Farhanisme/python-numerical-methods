...
def regula_falsi(a, b, tolerance=Decimal("0.00001"), max_iterations=10):
    a = Decimal(a)
    b = Decimal(b)
    iteration = 1
    ...
    while iteration <= max_iterations:
        ...
        print(f"a = {a}")
        print(f"b = {b}")
        # print(f"f(a) = f({a}) = 2({a})^3 + 3({a})^2 + 5({a}) + 15 = ({2*a**3} + {3*a**2} + {5*a} + 15) = {fa}")
        # print(f"f(b) = f({b}) = 2({b})^3 + 3({b})^2 + 5({b}) + 15 = ({2*b**3} + {3*b**2} + {5*b} + 15) = {fb}")
        # print(f"c = b - ( ((f(b) * (a - b)) / (f(a) - f(b)) ) = {b} - ( ({fb} * ({a} - {b})) / ({fa} - {fb}) ) = {c}")
        print(f"f(a) = ({6*a**5} + {3*a**4} + {1*a**6} + 3)")
        print(f"f(a) = {fa}")
        print(f"f(b) = ({6*b**5} + {3*b**4} + {1*b**6} + 3)")
        print(f"f(b) = {fb}")
        print(f"c = {b} - ({fb} * ({a} - {b})) / ({fa} - {fb}) ) \n= {c}")
        # print(f"f(c) = f({c}) = 2({c})^3 + 3({c})^2 + 5({c}) + 15 = ({2*c**3} + {3*c**2} + {5*c} + 15) \n= {fc}")
...
regula_falsi(a=0, b=-5, tolerance=Decimal("0.00001"))
